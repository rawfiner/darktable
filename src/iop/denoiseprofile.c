...
typedef struct dt_iop_denoiseprofile_params_t
{
  ...
  float nbhood;     // search radius
  float strength;   // noise level after equalization
  float force_red;  // nlmeans param to adjust blur amount on red channel
  float force_green; // nlmeans param to adjust blur amount on green channel
  float force_blue;  // nlmeans param to adjust blur amount on blue channel
  float scattering; // spread the patch search zone without increasing number of patches
...
typedef struct dt_iop_denoiseprofile_gui_data_t
{
  ...
  GtkWidget *force_red;
  GtkWidget *force_green;
  GtkWidget *force_blue;
...
void reload_defaults(dt_iop_module_t *module)
{
  ...
  if(g)
  {
    ...
    ((dt_iop_denoiseprofile_params_t *)module->default_params)->force_red = 1.0f;
    ((dt_iop_denoiseprofile_params_t *)module->default_params)->force_green = 1.0f;
    ((dt_iop_denoiseprofile_params_t *)module->default_params)->force_blue = 1.0f;
                   ...
                   dt_dev_pixelpipe_iop_t *piece)
{
  ...
  d->force_red = p->force_red;
  d->force_green = p->force_green;
  d->force_blue = p->force_blue;
  for(int i = 0; i < 3; i++)
  {
    d->a[i] = p->a[i];
    d->b[i] = p->b[i];
...
static void force_red_callback(GtkWidget *w, dt_iop_module_t *self)
{
  dt_iop_denoiseprofile_params_t *p = self->params;
  p->force_red = dt_bauhaus_slider_get(w);
  dt_dev_add_history_item(darktable.develop, self, TRUE);
}

static void force_green_callback(GtkWidget *w, dt_iop_module_t *self)
{
  dt_iop_denoiseprofile_params_t *p = self->params;
  p->force_green = dt_bauhaus_slider_get(w);
  dt_dev_add_history_item(darktable.develop, self, TRUE);
}

static void force_blue_callback(GtkWidget *w, dt_iop_module_t *self)
{
  dt_iop_denoiseprofile_params_t *p = self->params;
  p->force_blue = dt_bauhaus_slider_get(w);
  dt_dev_add_history_item(darktable.develop, self, TRUE);
}
...
void gui_update(dt_iop_module_t *self)
{
  ...
  dt_bauhaus_slider_set_soft(g->force_red, p->force_red);
  dt_bauhaus_slider_set_soft(g->force_green, p->force_green);
  dt_bauhaus_slider_set_soft(g->force_blue, p->force_blue);
...
void gui_init(dt_iop_module_t *self)
{
  ...
  g->force_red = dt_bauhaus_slider_new_with_range(self, 0.001f, 4.0f, .01, 1.0f, 3);
  g->force_green = dt_bauhaus_slider_new_with_range(self, 0.001f, 4.0f, .01, 1.0f, 3);
  g->force_blue = dt_bauhaus_slider_new_with_range(self, 0.001f, 4.0f, .01, 1.0f, 3);
  dt_bauhaus_slider_enable_soft_boundaries(g->force_red, 0.0001f, 1000.0f);
  dt_bauhaus_slider_enable_soft_boundaries(g->force_green, 0.0001f, 1000.0f);
  dt_bauhaus_slider_enable_soft_boundaries(g->force_blue, 0.0001f, 1000.0f);
  dt_bauhaus_slider_set_stop(g->force_red, 0.0f, 0.8f, 0.0f, 0.0f);
  dt_bauhaus_slider_set_stop(g->force_red, 1.0f, 0.8f, 0.0f, 0.0f);
  dt_bauhaus_slider_set_stop(g->force_green, 0.0f, 0.0f, 0.8f, 0.0f);
  dt_bauhaus_slider_set_stop(g->force_green, 1.0f, 0.0f, 0.8f, 0.0f);
  dt_bauhaus_slider_set_stop(g->force_blue, 0.0f, 0.0f, 0.0f, 0.8f);
  dt_bauhaus_slider_set_stop(g->force_blue, 1.0f, 0.0f, 0.0f, 0.8f);
  ...
  gtk_box_pack_start(GTK_BOX(g->box_nlm), g->strength, TRUE, TRUE, 0);
  gtk_box_pack_start(GTK_BOX(g->box_nlm), g->force_red, TRUE, TRUE, 0);
  gtk_box_pack_start(GTK_BOX(g->box_nlm), g->force_green, TRUE, TRUE, 0);
  gtk_box_pack_start(GTK_BOX(g->box_nlm), g->force_blue, TRUE, TRUE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(g->box_wavelets), g->strength, TRUE, TRUE, 0);
  ...
  dt_bauhaus_widget_set_label(g->force_red, NULL, _("red force"));
  dt_bauhaus_widget_set_label(g->force_green, NULL, _("green force"));
  dt_bauhaus_widget_set_label(g->force_blue, NULL, _("blue force"));
  ...
  gtk_widget_set_tooltip_text(g->force_red, _("finetune denoising strength for red channel"));
  gtk_widget_set_tooltip_text(g->force_green, _("finetune denoising strength for green channel"));
  gtk_widget_set_tooltip_text(g->force_blue, _("finetune denoising strength for blue channel"));
  ...
  g_signal_connect(G_OBJECT(g->force_red), "value-changed", G_CALLBACK(force_red_callback), self);
  g_signal_connect(G_OBJECT(g->force_green), "value-changed", G_CALLBACK(force_green_callback), self);
  g_signal_connect(G_OBJECT(g->force_blue), "value-changed", G_CALLBACK(force_blue_callback), self);
}
