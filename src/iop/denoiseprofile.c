                            ...
                            const dt_iop_roi_t *const roi_out)
{
  ...
  for(int kj_index = -K; kj_index <= K; kj_index++)
  {
    for(int ki_index = -K; ki_index <= K; ki_index++)
    {
      ...
      int kj = (kj_index * kj_index + abs(ki_index) * sqrt(abs(kj_index))) * sign(kj_index) * d->scattering + kj_index;
      int ki = (ki_index * ki_index + abs(kj_index) * sqrt(abs(ki_index))) * sign(ki_index) * d->scattering + ki_index;
                              ...
                              const dt_iop_roi_t *const roi_out)
{
  ...
  for(int kj_index = -K; kj_index <= 0; kj_index++)
  {
    for(int ki_index = -K; ki_index <= K; ki_index++)
    {
      ...
      // - ensuring that no patch can appear twice (provided that d->nbhood is in 0,1 range)
      // - avoiding grid artifacts by trying to take patches on various lines and columns
      int j = (kj_index * kj_index + abs(ki_index) * sqrt(abs(kj_index))) * sign(kj_index) * d->scattering + kj_index;
      int i = (ki_index * ki_index + abs(kj_index) * sqrt(abs(ki_index))) * sign(ki_index) * d->scattering + ki_index;
      int q[2] = { i, j };
