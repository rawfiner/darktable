...
typedef enum dt_iop_localcontrastrgb_guide_channel_t
{
  DT_LOCALCONTRAST_RGB_R = 0,    // $DESCRIPTION: "red"
  DT_LOCALCONTRAST_RGB_G = 1,    // $DESCRIPTION: "green"
  DT_LOCALCONTRAST_RGB_B = 2     // $DESCRIPTION: "blue"
} dt_iop_localcontrastrgb_guide_channel_t;

typedef enum dt_iop_localcontrastrgb_mode_t
{
  DT_LOCALCONTRAST_RGB_MODE_STANDARD = 0,  // $DESCRIPTION: "standard"
  DT_LOCALCONTRAST_RGB_MODE_DARKEN = 1,    // $DESCRIPTION: "darken only"
  DT_LOCALCONTRAST_RGB_MODE_BRIGHTEN = 2   // $DESCRIPTION: "brighten only"
} dt_iop_localcontrastrgb_mode_t;

typedef struct dt_iop_localcontrastrgb_params_t
{
  dt_iop_localcontrastrgb_guide_channel_t guide_channel; // $DEFAULT: DT_LOCALCONTRAST_RGB_G $DESCRIPTION: "guide"
  float radius; // $MIN: 1 $MAX: 500 $DEFAULT: 5 $DESCRIPTION: "radius"
  float strength; // $MIN: 0 $MAX: 4 $DEFAULT: 0.5 $DESCRIPTION: "strength"
  dt_iop_localcontrastrgb_mode_t mode; // $DEFAULT: DT_LOCALCONTRAST_RGB_MODE_STANDARD $DESCRIPTION: "correction mode"
...
static void normalize_manifolds(const float *const restrict blurred_in, float *const restrict blurred_manifold_lower, float *const restrict blurred_manifold_higher, const size_t width, const size_t height, const dt_iop_localcontrastrgb_guide_channel_t guide)
{
#ifdef _OPENMP
...
static void get_manifolds(const float* const restrict in, const size_t width, const size_t height,
                          ...
                          const dt_iop_localcontrastrgb_guide_channel_t guide,
...
static void apply_correction(const float* const restrict in,
                          ...
                          const dt_iop_localcontrastrgb_guide_channel_t guide,
                          const dt_iop_localcontrastrgb_mode_t mode,
...
dt_omp_firstprivate(in, width, height, guide, manifolds, out, sigma, mode) \
  ...
  for(size_t k = 0; k < width * height; k++)
  {
    ...
    for(size_t kc = 0; kc <= 1; kc++)
    {
      ...
      switch(mode)
      {
        case DT_LOCALCONTRAST_RGB_MODE_STANDARD:
          out[k * 4 + c] = outp;
          break;
        case DT_LOCALCONTRAST_RGB_MODE_DARKEN:
          out[k * 4 + c] = fminf(outp, pixelc);
          break;
        case DT_LOCALCONTRAST_RGB_MODE_BRIGHTEN:
...
{
  dt_iop_localcontrastrgb_params_t *d = (dt_iop_localcontrastrgb_params_t *)piece->data;
...
void reload_defaults(dt_iop_module_t *module)
{
  ...
  d->guide_channel = DT_LOCALCONTRAST_RGB_G;
  d->radius = 5.0f;
  d->strength = 0.5f;
  d->mode = DT_LOCALCONTRAST_RGB_MODE_STANDARD;
